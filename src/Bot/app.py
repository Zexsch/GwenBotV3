...
class App(commands.Bot):
    ...
    async def setup_hook(self) -> None:
        ...
        await self.add_cog(
            ListenerCog(bot=self, database=self.database, logger=self.logger)
        )
        await self.add_cog(
            GwensubCog(bot=self, database=self.database, logger=self.logger)
        )
        await self.add_cog(
            OwnerCog(bot=self, database=self.database, logger=self.logger)
        )
        await self.add_cog(
            WinrateCog(
        ...
        await self.add_cog(
            DeepseekCog(bot=self, database=self.database, logger=self.logger)
        )
