...
from Config.config import MESSAGE_CHANNEL, OWNER_ID
...
class LeaderboardCog(commands.Cog):
    ...
    async def questions(self, ctx: commands.Context):
        await ctx.send(
            f"The current amount of question marks is {self.database.fetch_amount()}."
        )
    ...
    async def questions_user(self, ctx: commands.Context, id):
        await ctx.send(
            f"Command is currently being reworked. Please be patient as I work on making it not bad..."
        )
        #
        # try:
        #    id = int(id)
